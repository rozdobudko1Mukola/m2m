...
class TestObjectsGroup:
    ...
    def test_create_a_new_group_of_objects_m2m_396(self, user_page, create_and_remove_units_by_api, delete_device_groups_after_test):
        """ ||M2M-396|| Створити нову групу обєктів """
        objects_page = ObjectsPage(user_page)
    ...
    @pytest.mark.parametrize("user_page", ["SELFREG"], indirect=True)
    def test_add_objects_to_a_group_of_objects_m2m_1542(self, user_page, create_and_remove_units_by_api, create_and_del_device_groups):
        """ ||M2M-1542|| Додати об'єкти до групи обєктів """
        objects_page = ObjectsPage(user_page)
    ...
    @pytest.mark.parametrize("user_page", ["SELFREG"], indirect=True)
    def test_remove_objects_from_a_group_of_objects_m2m_1543(self, user_page, create_and_remove_units_by_api, create_and_del_device_groups):
        """ ||M2M-1543|| Видалити об'єкти з групи обєктів """
        objects_page = ObjectsPage(user_page)
    ...
    @pytest.mark.parametrize("user_page", ["SELFREG"], indirect=True)
    def test_open_the_object_properties_window_in_the_list_of_group_members_m2m_407(self, user_page, create_and_del_device_groups, create_and_remove_units_by_api):
        """ ||M2M-407|| Відкрити вікно властивості об'єкта в списку учасників групи """
        objects_page = ObjectsPage(user_page)
...
class TestInteractionWithObjects:
    ...
    def test_search_unit_in_the_group_popup_valid_data(self, user_page, test_data, full_unit_create_and_remove_by_api):
        ...
        expect(objects_page.search_unit_in_group(test_data["device_name"][18])).to_have_count(2) # очікуємо 1 об'єкт. 2 - це тому що спіску завжди одна пуста li
    ...
    def test_search_unit_in_the_group_popup_use_not_full_data(self, user_page, test_data, full_unit_create_and_remove_by_api):
        ...
        expect(objects_page.search_unit_in_group(' '.join(test_data["device_name"][18].split()[-2:]))).to_have_count(2) # очікуємо 1 об'єкт. 2 - це тому що спіску завжди одна пуста li
    ...
    def test_search_unit_in_the_group_popup_use_invalid_data(self, user_page, test_data, full_unit_create_and_remove_by_api):
        ...
        expect(objects_page.search_unit_in_group("qwerty123")).to_have_count(1) # очікуємо 1 об'єкт. 2 - це тому що спіску завжди одна пуста li
...
# # Search for units using a filter-------------------------------------------------------------------------------------------------------------------------------------
# This section contains tests for searching units using various filters such as name, unique ID, phone number, account, tracker model, and custom fields.
# Each test verifies the functionality of the search filters with valid, partially valid, and invalid inputs.
...
# Filter Пошук за параметром "Адміністративні поля" Повною валідною назвою
    ...
    @pytest.mark.parametrize("user_page", ["SELFREG"], indirect=True)
    @pytest.mark.parametrize("full_unit_create_and_remove_by_api", [3], indirect=True)
    def test_search_for_an_object_using_filters_T0a3bed46(self, user_page, test_data, full_unit_create_and_remove_by_api):
        objects_page = ObjectsPage(user_page)
