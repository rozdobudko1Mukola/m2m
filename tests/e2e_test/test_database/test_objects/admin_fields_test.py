...
def revove_admin_filds(api_context: APIRequestContext, token: str, test_data):
    ...
    if device_ids:
        response = device_api.update_admin_fields_for_device(device_ids[0], adminFields="{}")  # Clear admin fields for the first device
...
class TestCustomFields:
    ...
    def test_open_custom_fields_tab(self, user_page, full_unit_create_and_remove_by_api, revove_admin_filds):
        ...
        expect(objects_page.object_popap_tablist["admin_f"]).to_be_visible() # Check that the tab is opened
    ...
    def test_fill_only_name_fild(self, user_page, full_unit_create_and_remove_by_api, revove_admin_filds):
        ...
        expect(objects_page.object_popap_tablist["admin_f"]).to_be_focused() # Check that the tab is opened
        expect(custom_fields_page.save_btn).to_be_disabled() # Check that the save button is disabled
    ...
    @pytest.mark.parametrize("user_page", ["SELFREG"], indirect=True)
    @pytest.mark.parametrize("full_unit_create_and_remove_by_api", [1], indirect=True)
    def test_check_modal_window_using_valid_data(self, user_page, full_unit_create_and_remove_by_api, revove_admin_filds):
    ...
    @pytest.mark.parametrize("user_page", ["SELFREG"], indirect=True)
    @pytest.mark.parametrize("full_unit_create_and_remove_by_api", [1], indirect=True)
    def test_fill_custom_fields_tab_invalid_length(self, user_page, full_unit_create_and_remove_by_api, revove_admin_filds):
